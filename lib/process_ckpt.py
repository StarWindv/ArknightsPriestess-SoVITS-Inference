...
def load_sovits_new(sovits_path):
    import sys
    print(f"[load_sovits_new] Loading: {sovits_path}", file=sys.stderr)
    print(f"[load_sovits_new] File size: {os.path.getsize(sovits_path)} bytes", file=sys.stderr)
    f = open(sovits_path, "rb")
    meta = f.read(2)
    print(f"[load_sovits_new] Header: {meta}", file=sys.stderr)
    if meta != b"PK":
        rest = f.read()
        f.close()
        print(f"[load_sovits_new] Rest size: {len(rest)} bytes", file=sys.stderr)
        data = b"PK" + rest
        del rest
        print(f"[load_sovits_new] Data size: {len(data)} bytes, creating BytesIO...", file=sys.stderr)
        bio = BytesIO()
        bio.write(data)
        del data
        bio.seek(0)
        print(f"[load_sovits_new] BytesIO ready, calling torch.load...", file=sys.stderr)
