...
sys.path.insert(0, os.path.join(now_dir, "lib"))
sys.path.insert(0, os.path.join(now_dir, "lib", "eres2net"))
import os
...
class TTS_Config:
    ...
    def __init__(self, config: Union[dict, str] = ""):
        # 设置默认配置文件路径
        configs_base_path: str = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "configs")
...
class TTS:
    ...
    def init_vocoder(self, version: str):
        if version == "v3":
            ...
            self.vocoder = BigVGAN.from_pretrained(
                os.path.join(now_dir, "models", "models--nvidia--bigvgan_v2_24khz_100band_256x"),
            ...
            state_dict_g = torch.load(
                os.path.join(now_dir, "models", "gsv-v4-pretrained", "vocoder.pth"),
                map_location="cpu",
                weights_only=False,
            )
            logger.debug("loading vocoder", self.vocoder.load_state_dict(state_dict_g))
    ...
    def init_sr_model(self):
        ...
        try:
            ...
            logger.debug(i18n("你没有下载超分模型的参数，因此不进行超分。如想超分请先参照教程把文件下载好"))
    ...
    def run(self, inputs: dict):
        ...
        if split_bucket and speed_factor == 1.0 and not (self.configs.use_vocoder and parallel_infer):
            ...
            logger.debug(i18n("当开启并行推理模式时，SoVits V3/4模型不支持分桶处理，已自动关闭分桶处理"))
            split_bucket = False
        else:
